...
{ 
  ...
  double size = 0.01, weight = 0.00001; // センサのサイズ[m]と重量[kg]e
...
void makepointer(){
  dMass mass;
  dVector3 pos, pos1, pos2, pos3;
  dBodyGetRelPointPos(box[0].body, 0.5*lx, -0.5*ly, 0, pos);
  dBodyGetRelPointPos(box[0].body, -0.5*lx, -0.5*ly, 0, pos1);
  dBodyGetRelPointPos(box[1].body, -0.5*lx, 0.5*ly, 0, pos2);
  dBodyGetRelPointPos(box[1].body, -0.5*lx, -0.5*ly, 0, pos3);
  pointer[0]= dBodyCreate(world);
  pointer[1]= dBodyCreate(world);
  pointer[2]= dBodyCreate(world);
  pointer[3]= dBodyCreate(world);
  dBodySetPosition(pointer[0], pos[0], pos[1], pos[2]);
  dBodySetPosition(pointer[1], pos1[0], pos1[1], pos1[2]);
  dBodySetPosition(pointer[2], pos2[0], pos2[1], pos2[2]);
  dBodySetPosition(pointer[3], pos3[0], pos3[1], pos3[2]);
  for (int i = 0; i < 4; i++){
  dMassSetZero(&mass);
  dMassSetBoxTotal(&mass, 0.00001, 0.1, 0.1, 0.1);
  dBodySetMass(pointer[i], &mass);

  pointer_joint[i] = dJointCreateFixed(world, 0); // 固定ジョイントの生成
  if (i == 0 || i == 1){
    dJointAttach(pointer_joint[i], box[0].body, pointer[i]); // 先端リンクと結合
  }else {
    dJointAttach(pointer_joint[i], box[1].body, pointer[i]); // 先端リンクと結合
  }
  dJointSetFixed(pointer_joint[i]);
  }
...
static void destroybox(){
  ...
  dGeomDestroy(box[1].geom);
    for (int i = 0; i <4; i++){
    dJointDestroy(pointer_joint[i]);
    dBodyDestroy(pointer[i]);
 }
}
...
void drawpointer()
{
   dsSetColor(0,0,1);
  for (int i = 0; i < 4; i++){
   dBodyGetRotation(pointer[i]);
   dsDrawSphere(dBodyGetPosition(pointer[i]),dBodyGetRotation(pointer[i]),0.02);
  }
}
...
void catchball(){
  ...
  if (!fast_mode && dist < 0.05 && ACTION!=0){
    fast_mode = true;
  ...
  if (fast_mode){
    ...
    case 5:
      ...
      break;
    case 13:
      // dBodyGetRelPointPos(box[0].body, 0.5*lx, 0.5*ly, 0.2*lz, target_pos);
      // get_goal_target(box[0].body, 0.5*lx, 0.5*ly, 0.2*lz, target_pos);
    ...
    case 6:
      ...
      break;
    case 14:
      // dBodyGetRelPointPos(box[0].body, 0.25*lx, 0.25*ly, 0.2*lz, target_pos);
      // get_goal_target(box[0].body, 0.25*lx, 0.25*ly, 0.2*lz, target_pos);
    ...
    // default: //デフォは中心向き
    // dBodyGetRelPointPos(box[0].body, 0,0,0.2*lz,target_pos);
    //   break;
    }
    fast_mode_counter--;
...
if (!pause){
  ...
  // calculatestickness_side(box[0].body, box[1].body);
  // double dist_x = calculateX(box[0].body, box[1].body);
  // double dist_y = calculateX(box[0].body, box[1].body);
  // if (dist_y > 0){
  //   calculatestickness(box[0].body, box[1].body);
  // }else{
  //   calculatestickness(box[1].body, box[0].body);
  // }
  // if (dist_x < 0){
  //   calculatestickness_side(box[0].body, box[1].body);
  // }else{
  //   calculatestickness_side(box[1].body, box[0].body);
  // }
  inverseKinematics();
  printSensorPosition();
  Pcontrol();                                  // P制御
...
void command2(int cmd)
{
  ...
  case 'z':  dBodyAddForceAtRelPos(box[0].body, 10, 0, 0, -0.5*lx, -0.5*ly, 0);
