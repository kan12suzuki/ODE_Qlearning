...
#endif
#define NUM 4                          // リンク数
#define F "./Qtable_6p_13/Sumrewards.csv"
#define F2 "./Qtable_6p_13/ep_vs_st.csv"
#define F3 "./Qtable_6p_13/Qtable.csv"
#define F4 "./Qtable_6p_13/eval_Rewards.csv"
#define F5 "./Qtable_6p_13/Qtable_episode" 
double t = 0.0;
...
// 描画用パラメータ
std::mutex world_mutex;                 // ODEワールド保護用
double link_r[NUM];
double link_length[NUM];
...
// Stickiness penalty weights (tunable) -- reduced
double STICK_PENALTY_LEARN = 2.0;
double STICK_PENALTY_EVAL = 6.0;
// Normalization baseline for avgSpreadDiff
...
class Stickness{
  ...
  public:
    ...
    bool terminate(){
      ...
      // double y5 = state[9];
      // double y6 = state[11];
      // terminate should be based on absolute (world) positions of particles
      int num_particles = state.size()/2;
      double threshold = x_dst_lim;
      double threshold_sq = threshold * threshold;

      // check out-of-range using world coordinates
      for (int i = 0; i < num_particles; ++i){
        const dReal* pos = dBodyGetPosition(box[i].body);
        double range_sq = pos[0]*pos[0] + pos[1]*pos[1];
        if (range_sq > 2.5 * 2.5){
          return true;
        }
      }
      ...
      // check pairwise separations in world coordinates
      for (int i = 0; i < num_particles; ++i){
  ...
  void calculatestickness(dBodyID body1, dBodyID body2) {//body1が上側、body2が下側の粒
    ...
    // if (fabs(relPos[0]) <= lx*1.1 && fabs(relPos[1]) <= ly*1.1) {
      ...
      if (force2[1] < -flim) {
        if (force2[0] > 0) {
          dBodyAddForceAtRelPos(body1, relPos[0], force2[1]/2 + relPos[1] , 0, 0.5*lx, -0.5*ly, 0, 0.5*lx, -0.5*ly, 0);
  ...
  double calculate_total_spread(const std::vector<double>& obs) {
    ...
    // 各粒について最小近傍距離が閾値を超えていれば「分離している」とみなす
    // 返り値は [0,1] の割合（分離している粒の割合）
    double calculate_separation_score(const std::vector<double>& obs) {
      ...
      // 各粒の最も近い粒までの距離の分散を計算する
      // 値が大きいほど近傍距離にばらつき（＝一部の粒がより離れている）ことを示す
...
public:
  ...
  int getAction(const std::vector<double>& state, int episode) {
        ...
        double epsilon = 0.5 / (1.0 + episode * 0.01);
...
static void simLoop(int pause){
  ...
  for (int i = 0; i < NUM; ++i){
    if (rlink[i].body){
      ...
      const dReal *R = dBodyGetRotation(rlink[i].body);
      dsSetColor(1.0, 1.0, 1.0);
      dsDrawCapsule(pos, R, link_length[i], link_r[i]);
    }
  }
  // draw boxes (up to 6)
  for (int k = 0; k < 6; ++k){
    if (box[k].body){
      const dReal *pos = dBodyGetPosition(box[k].body);
      const dReal *R = dBodyGetRotation(box[k].body);
      dVector3 dim = {box[k].lx, box[k].ly, box[k].lz};
      dsSetColor(0.2, 0.8, 0.2);
      dsDrawBox(pos, R, dim);
    }
  }
  // draw ground plane
  dsSetColor(0.6, 0.6, 0.6);
  dReal gpos[3] = {0,0,-0.01};
  dReal gR[12];
  for (int i=0;i<12;i++) gR[i] = 0;
  gR[0]=gR[4]=gR[8]=1.0;
  dVector3 gdim = {10.0, 10.0, 0.02};
  dsDrawBox(gpos, gR, gdim);

  // draw sensor (if exists)
  if (sensor){
    const dReal *spos = dBodyGetPosition(sensor);
    const dReal *sR = dBodyGetRotation(sensor);
    dsSetColor(1.0, 0.5, 0.0);
    dsDrawSphereD(spos, sR, 0.02);
  }
}

// start callback to set camera
static void start(){
  static float xyz[3] = {3.0f, 0.0f, 1.0f};
  static float hpr[3] = {-180.0f, 0.0f, 0.0f};
...
int main(int argc, char *argv[]) 
{
  ...
  std::thread trainer([&](){
    ...
    while(true){
      episode ++;
      // create world objects (arm, sensor, boxes) under lock briefly
      ...
      for (int t = 0; t < max_number_of_steps; ++t){
        ...
        auto observation = env.getcurrentState();
        ...
        if (env.terminate() || t == max_number_of_steps -1){
          ...
          for (int i = 0; i < num_particles; ++i){
            const dReal* p = dBodyGetPosition(box[i].body);
          ...
          if (is_out_of_range){ reward += -20.0; }
          else if(is_separated){ reward += 200; success_flag++; }
          else{
        ...
        // --- Penalty: if particle is out of the arm's reachable workspace (arm fully stretched) ---
        {
          std::lock_guard<std::mutex> lock(world_mutex);
          const dReal *spos = dBodyGetPosition(sensor);
          double base_height = l[0] + l[1];
          double maxReach = l[2] + l[3];
          bool unreachable = false;
          int num_p = observation.size() / 2;
          for (int bi = 0; bi < num_p; ++bi){
            const dReal *bpos = dBodyGetPosition(box[bi].body);
            double dist_base = sqrt(bpos[0]*bpos[0] + bpos[1]*bpos[1] + (bpos[2]-base_height)*(bpos[2]-base_height));
            double dist_sensor = sqrt((spos[0]-bpos[0])*(spos[0]-bpos[0]) + (spos[1]-bpos[1])*(spos[1]-bpos[1]) + (spos[2]-bpos[2])*(spos[2]-bpos[2]));
            // If the particle is beyond geometric reach and the end-effector is still far, apply small penalty
            if (dist_base > maxReach + 0.05 && dist_sensor > 0.25) { unreachable = true; break; }
          }
          if (unreachable) {
            reward += -5.0; // small penalty, do not terminate
            // std::cout << "Applied unreachable penalty" << std::endl;
          }
        }

        int nextstate = qTable.digitizeState(env.getcurrentState());
        int nextAction = action;
        if (t%100 ==0){ nextAction = qTable.getAction(env.getcurrentState(), episode); }
        ...
        if (env.terminate() || t == max_number_of_steps-1){
          std::cout << episode << " finished after " << t+1 << " time steps" << std::endl;
          std::cout << "  Episode reward: " << sumReward << std::endl;
          std::ofstream outputfile(F, std::ios::app);
          outputfile << episode << "," << sumReward <<"\n";
          sumReward = 0.0;

            // --- write per-episode metrics (min pair distance, mean pair distance, sep_score, mean nearest) ---
            {
              const std::vector<double>& obs_ref = observation;
              int num_particles = obs_ref.size() / 2;
              int num_pairs = std::max(1, num_particles * (num_particles - 1) / 2);
              double total_dist = 0.0;
              double min_pair = 1e9;
              std::vector<double> min_dists(num_particles, 1e9);
              for (int i = 0; i < num_particles; ++i){
                for (int j = i+1; j < num_particles; ++j){
                  double dx = obs_ref[2*i] - obs_ref[2*j];
                  double dy = obs_ref[2*i+1] - obs_ref[2*j+1];
                  double d = std::sqrt(dx*dx + dy*dy);
                  total_dist += d;
              ...
              double mean_pair = total_dist / static_cast<double>(num_pairs);
              double mean_min_dist = 0.0;
              for (double v : min_dists) mean_min_dist += v;
              mean_min_dist /= static_cast<double>(num_particles);
              double sep_score = env.calculate_separation_score(obs_ref);

              if (episode == 1){
                std::ofstream mhead("./Qtable_6p_6/metrics.csv", std::ios::app);
                mhead << "episode,min_pair,mean_pair,sep_score,mean_min_dist\n";
                mhead.close();
              }
              std::ofstream mfile("./Qtable_6p_6/metrics.csv", std::ios::app);
              mfile << episode << "," << min_pair << "," << mean_pair << "," << sep_score << "," << mean_min_dist << "\n";
              mfile.close();
            }
      ...
      if (episode % interval_episodes == 0){
        ...
        for (int eval_t = 0; eval_t < max_number_of_steps; ++eval_t){
          ...
          if (env.terminate() || eval_t == max_number_of_steps -1){
            ...
            if (is_out_of_range) eval_reward += -20.0;
          ...
          // --- Penalty in eval: if particle is out of the arm's reachable workspace ---
          {
            std::lock_guard<std::mutex> lock(world_mutex);
            const dReal *spos = dBodyGetPosition(sensor);
            double base_height = l[0] + l[1];
            double maxReach = l[2] + l[3];
            bool unreachable_eval = false;
            int num_p = eval_observation.size() / 2;
            for (int bi = 0; bi < num_p; ++bi){
              const dReal *bpos = dBodyGetPosition(box[bi].body);
              double dist_base = sqrt(bpos[0]*bpos[0] + bpos[1]*bpos[1] + (bpos[2]-base_height)*(bpos[2]-base_height));
              double dist_sensor = sqrt((spos[0]-bpos[0])*(spos[0]-bpos[0]) + (spos[1]-bpos[1])*(spos[1]-bpos[1]) + (spos[2]-bpos[2])*(spos[2]-bpos[2]));
              if (dist_base > maxReach + 0.05 && dist_sensor > 0.25) { unreachable_eval = true; break; }
            }
            if (unreachable_eval) eval_reward += -5.0;
          }
          int eval_nextstate = qTable.digitizeState(env.getcurrentState());
          int eval_nextAction = eval_action;
          if (eval_t%100 ==0) eval_nextAction = qTable.greedyAction(env.getcurrentState());
          eval_sumReward_local += std::pow(ganma, eval_t)*eval_reward;

          if (env.terminate() || eval_t == max_number_of_steps-1){
            std::ofstream evalfile(F4, std::ios::app);
            evalfile << episode << "," << eval_sumReward_local <<"\n";
            evalfile.close();
            {
              std::lock_guard<std::mutex> lock(world_mutex);
              dJointGroupDestroy(contactgroup);
              for (int i = 0; i < 6; i++) memset(&feedback[i], 0, sizeof(dJointFeedback));
              destroyarm();
              destroybox();
            }
            break;
  ...
  // No draw: wait for trainer to finish
...
class Stickness{
  ...
  void calculatestickness(dBodyID body1, dBodyID body2) { /* ... (kept) */ }
  void calculatestickness_side(dBodyID body1, dBodyID body2) { /* ... (kept) */ }
  int judge_relation_local(int i, int j){ /* ... (kept) */ return 0; }
  double calculate_total_spread(const std::vector<double>& obs){ /* ... (kept) */ return 0.0; }
  double calculate_separation_score(const std::vector<double>& obs){ /* ... (kept) */ return 0.0; }
  double calculate_nearest_distance_variance(const std::vector<double>& obs){ /* ... (kept) */ return 0.0; }

  double step(int action){ return 0.0; }
  std::vector<double> getcurrentState(){ return std::vector<double>(); }
  std::vector<double> reset(){ return std::vector<double>(12,0.0); }
  std::vector<double> eval_reset(){ return std::vector<double>(12,0.0); }
};

// Note: For brevity some duplicated implementations are elided above since
// this no-draw file uses the same training thread logic below and only
// needs main to skip drawstuff.
...
int main(int argc, char *argv[]) 
{
  ...
  // Start training in background thread (re-using same training code as original)
  std::thread trainer([&](){
    // For simplicity reuse much of the original training routine; in practice
    // you can copy the full trainer body from the original file.
    Stickness env;
    // simplified loop to mirror original behaviour
    for (int episode=1; episode<=1; ++episode){
      // placeholder: original trainer body runs here
    }
  });

  // No drawing: just wait for trainer to finish
  if (trainer.joinable()) trainer.join();
