...
#endif
#define NUM 4                          // リンク数
#define F "./Qtable_726p_1/Sumrewards.csv"
#define F2 "./Qtable_726p_1/ep_vs_st.csv"
#define F3 "./Qtable_726p_1/Qtable.csv"
#define F4 "./Qtable_726p_1/eval_Rewards.csv"
#define F5 "./Qtable_726p_1/Qtable_episode" 
double t = 0.0;
...
void catchball(){
  ...
  switch(ACTION){
    ...
    case 14:
    case 15:
    dBodyGetRelPointPos(box[1].body, 0, 0.5*ly+0.5*r, 0.2*lz, target_pos); break;
    case 16:
    case 17:
    case 18:
    ...
    case 26:
    case 27:
    dBodyGetRelPointPos(box[2].body, 0, 0.5*ly+0.5*r, 0.2*lz, target_pos); break;
    case 28:
    case 29:
    case 30:
    dBodyGetRelPointPos(box[2].body, -0.5*lx-0.5*r, 0, 0.2*lz, target_pos); break;
    case 31:
    case 32:
    case 33:
    dBodyGetRelPointPos(box[2].body, 0.5*lx+0.5*r, 0, 0.2*lz, target_pos); break;
    ...
    case 50:
    case 51:
    dBodyGetRelPointPos(box[4].body, 0, 0.5*ly+0.5*r, 0.2*lz, target_pos); break;
    case 52:
    case 53:
    case 54:
  ...
  if (fast_mode){
    switch (ACTION)
    {
      ...
      get_local_goal(box[0].body, 0, -0.5*ly-0.5*r, 0.2*lz, 0.5*lx, 0.5*ly, 0.2*lz, target_pos); break;
      case 13:
      get_local_goal(box[1].body, 0, 0.5*ly+0.5*r, 0.2*lz, 0.5*lx, -0.5*ly, 0.2*lz, target_pos); break;
      case 14:
      get_local_goal(box[1].body, 0, 0.5*ly+0.5*r, 0.2*lz, 0, -0.5*ly, 0.2*lz, target_pos); break;
      case 15:
      get_local_goal(box[1].body, 0, 0.5*ly+0.5*r, 0.2*lz, -0.5*lx, -0.5*ly, 0.2*lz, target_pos); break;
      case 16:
      ...
      get_local_goal(box[3].body, 0, -0.5*ly-0.5*r, 0.2*lz, 0.5*lx, 0.5*ly, 0.2*lz, target_pos); break;
      case 49:
      get_local_goal(box[4].body, 0, 0.5*ly+0.5*r, 0.2*lz, 0.5*lx, -0.5*ly, 0.2*lz, target_pos); break;
      case 50:
      get_local_goal(box[4].body, 0, 0.5*ly+0.5*r, 0.2*lz, 0, -0.5*ly, 0.2*lz, target_pos); break;
      case 51:
      get_local_goal(box[4].body, 0, 0.5*ly+0.5*r, 0.2*lz, -0.5*lx, -0.5*ly, 0.2*lz, target_pos); break;
      case 52:
...
int main(int argc, char *argv[]) 
{
  ...
  std::thread trainer([&](){
    ...
    const int max_number_of_steps = 10000;
    ...
    while(true){
      ...
      for (int t = 0; t < max_number_of_steps; ++t){
        ...
        if (env.terminate() || t == max_number_of_steps -1){
          ...
          if (is_out_of_range){ reward += -20000.0; }
            ...
            if (separated_pairs > 0) {
              reward += separated_pairs * 20.0; // Reward each separated pair at episode end
        ...
        if (env.terminate() || t == max_number_of_steps-1){
          ...
          sumReward = 0.0;
            ...
            // --- write per-episode metrics (min pair distance, mean pair distance, sep_score, mean nearest) ---
            {
              ...
              if (episode == 1){
                std::ofstream mhead("./Qtable_726p_1/metrics.csv", std::ios::app);
                mhead << "episode,min_pair,mean_pair,sep_score,mean_min_dist\n";
                mhead.close();
              }
              std::ofstream mfile("./Qtable_726p_1/metrics.csv", std::ios::app);
      ...
      if (episode % interval_episodes == 0){
        ...
        for (int eval_t = 0; eval_t < max_number_of_steps; ++eval_t){
          ...
          if (env.terminate() || eval_t == max_number_of_steps -1){
            ...
            if (is_out_of_range) eval_reward += -20000.0;
              ...
              if (separated_pairs > 0) {
                eval_reward += separated_pairs * 20.0; // Reward each separated pair at episode end
              } else {
                eval_reward += -30.0;
              }
  ...
  // when user closes the window, join trainer and clean up
